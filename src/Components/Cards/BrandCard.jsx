...
const BrandCard = ({brand}) => {
    ...
    return (
        <div className={`flex flex-col items-stretch justify-between border-2 rounded-xl cursor-pointer`} onClick={() => navigate(`brands/${brand.name}`, { state: brand })}>
            ...
            <div className="flex flex-col gap-3 justify-between items-start  h-full p-5">
                <h3 className="textLg dark:text-white">{brand.name}</h3>
            </div>
        </div>
    );
};
